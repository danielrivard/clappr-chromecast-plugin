...
})(self, (__WEBPACK_EXTERNAL_MODULE__724__) => {
...
var _player = __webpack_require__(724);
var _chromecast = _interopRequireDefault(__webpack_require__(5690));
...
var ChromecastPlayback = /*#__PURE__*/function (_Playback) {
  _inherits(ChromecastPlayback, _Playback);
  var _super = _createSuper(ChromecastPlayback);
  function ChromecastPlayback(options) {
    var _this;
    _classCallCheck(this, ChromecastPlayback);
    _this = _super.call(this, options);
    _this.src = options.src;
    _this.currentMedia = options.currentMedia;
    _this.mediaControl = options.mediaControl;
    _this.currentMedia.addUpdateListener(function () {
    ...
    _this.settings["default"] && (_this.settings["default"] = _this.settings["default"].filter(noVolume));
    _this.settings.left && (_this.settings.left = _this.settings.left.filter(noVolume));
    _this.settings.right && (_this.settings.right = _this.settings.right.filter(noVolume));
    _this._closedCaptionsTracks = options.ccTracks || [];
    _this._ccTrackId = -1;
    _this._updateCCTrackID = options.updateCCTrackID;
    return _this;
  }
  _createClass(ChromecastPlayback, [{
    ...
    key: "template",
    get: function get() {
      return (0, _player.template)(_chromecast["default"]);
    }
  }, {
    key: "attributes",
    get: function get() {
    ...
    value: function pause() {
      ...
      if (this.getPlaybackType() === _player.Playback.LIVE) {
        this.trigger(_player.Events.PLAYBACK_DVR, true);
      }
    }
  }, {
    key: "stop",
    ...
    value: function seek(time) {
      ...
      this.currentMedia.seek(request, function () {
        ...
        return _player.Log.warn('seek failed');
      });
      if (this.getPlaybackType() === _player.Playback.LIVE) {
        // assume live if time within 30 seconds of end of live stream
        this.trigger(_player.Events.PLAYBACK_DVR, time < this.getDuration() - 30);
      }
    }
  }, {
    key: "seekPercentage",
    ...
    value: function getPlaybackType() {
      return this.currentMedia.liveSeekableRange ? _player.Playback.LIVE : _player.Playback.VOD;
    }
  }, {
    ...
    value: function onMediaStatusUpdate() {
      ...
      if (this.currentMedia.playerState === 'BUFFERING') {
        ...
        this.trigger(_player.Events.PLAYBACK_BUFFERING, this.name);
      } else if (this.currentMedia.playerState === 'PLAYING') {
        if (this.isBuffering) {
          this.isBuffering = false;
          this.trigger(_player.Events.PLAYBACK_BUFFERFULL, this.name);
        }
        if (this.prevState !== this.currentMedia.playerState) {
          this.trigger(_player.Events.PLAYBACK_PLAY, this.name);
        }
        ...
        this.trigger(_player.Events.PLAYBACK_ENDED, this.name);
      } else if (this.currentMedia.playerState === 'PAUSED') {
        if (this.prevState !== this.currentMedia.playerState) {
          this.trigger(_player.Events.PLAYBACK_PAUSE, this.name);
        }
      }
    ...
    value: function updateMediaControl() {
      ...
      this.trigger(_player.Events.PLAYBACK_TIMEUPDATE, {
        current: position,
        total: duration
      }, this.name);
    }
...
/* harmony export */ });
// Module
var code = "<div class=\"chromecast-playback-background\"></div>\n<div class=\"chromecast-playback-overlay\"></div>\n";
// Exports
...
__webpack_require__(489);
var _player = __webpack_require__(724);
var _chromecast_playback = _interopRequireDefault(__webpack_require__(5238));
var _style = _interopRequireDefault(__webpack_require__(2717));
var _lodash = _interopRequireDefault(__webpack_require__(1730));
var _ic_cast_24dp = _interopRequireDefault(__webpack_require__(1737));
var _ic_cast0_24dp = _interopRequireDefault(__webpack_require__(2914));
var _ic_cast1_24dp = _interopRequireDefault(__webpack_require__(2493));
var _ic_cast2_24dp = _interopRequireDefault(__webpack_require__(4134));
var _ic_cast_connected_24dp = _interopRequireDefault(__webpack_require__(3288));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, _toPropertyKey(descriptor.key), descriptor); } }
function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
...
var ChromecastPlugin = /*#__PURE__*/function (_UICorePlugin) {
  _inherits(ChromecastPlugin, _UICorePlugin);
  var _super = _createSuper(ChromecastPlugin);
  function ChromecastPlugin(core) {
    var _this;
    _classCallCheck(this, ChromecastPlugin);
    _this = _super.call(this, core);
    _this.bootTryDelay = _this.options.bootTryDelay || 500; // Default is 500 milliseconds between each attempt
    _this.bootMaxTryCount = _this.options.bootMaxTryCount || 6; // Default is 6 attempts (3 seconds)
    _this.bootTryCount = 0;
    _this.textTracks = [];
    _this.messageNamespace = _this.options.customNamespace || DEFAULT_MESSAGE_NAMESPACE;
    if (_this.isBootable()) {
      _this.appId = _this.options.appId || DEFAULT_CLAPPR_APP_ID;
      _this.deviceState = DEVICE_STATE.IDLE;
      _this.embedScript();
    } else {
      _this.disable();
    }
  ...
  _createClass(ChromecastPlugin, [{
    ...
    value: function bindEvents() {
      ...
      if (_player.Events.CORE_ACTIVE_CONTAINER_CHANGED) {
        // Clappr 0.3.0 or greater
        this.listenTo(this.core, _player.Events.CORE_ACTIVE_CONTAINER_CHANGED, this.containerChanged);
      } else {
        this.listenTo(this.core.mediaControl, _player.Events.MEDIACONTROL_CONTAINERCHANGED, this.containerChanged);
      }
      if (this.container) {
        this.listenTo(this.container, _player.Events.CONTAINER_TIMEUPDATE, this.containerTimeUpdate);
        this.listenTo(this.container, _player.Events.CONTAINER_PLAY, this.containerPlay);
        this.listenTo(this.container, _player.Events.CONTAINER_ENDED, this.sessionStopped);
      }
    }
  }, {
    key: "isBootable",
    ...
    value: function enable() {
      _get(_getPrototypeOf(ChromecastPlugin.prototype), "enable", this).call(this);
      this.render();
      this.embedScript();
    }
  }, {
    ...
    value: function embedScript() {
      ...
      if (!window.chrome || !window.chrome.cast || !window.chrome.cast.isAvailable) {
        ...
        script.onload = function () {
          return _this2.bootstrapCastApi();
        };
    ...
    value: function bootstrapCastApi() {
      ...
      if (this.bootTryCount > this.bootMaxTryCount) {
        this.bootTryCount = 0;
        _player.Log.warn('GCastApi bootstrap timeout');
        this.disable();
        return;
      }
      ...
      if (window.chrome) {
        ...
        if (window.chrome.cast && window.chrome.cast.isAvailable) {
          ...
          window['__onGCastApiAvailable'] = function (loaded, errorInfo) {
            if (loaded) {
              _this3.appId = _this3.appId || DEFAULT_CLAPPR_APP_ID;
              _this3.initializeCastApi();
            } else {
              _player.Log.warn('GCastApi error', errorInfo);
              _this3.disable();
            }
          };
        }
    ...
    value: function updateCCTrackID(trackID) {
      if (trackID !== -1) {
        if (this.textTracks.filter(function (t) {
          ...
          console.warn("Failed to enable text track with ID ".concat(trackID, ", as it does not exist."));
          return;
        }
      }
      ...
      if (this.session) {
        this.session.sendMessage("urn:x-cast:".concat(this.messageNamespace, ":active-text-tracks"), enabledTextTrackIDs);
      }
      var container = this.core.getCurrentContainer();
      if (container) {
        container.trigger(_player.Events.CONTAINER_SUBTITLE_CHANGED, {
          id: trackID
        });
      }
    }
    ...
    value: function initializeCastApi() {
      ...
      chrome.cast.initialize(apiConfig, function () {
        return _player.Log.debug(_this4.name, 'init success');
      }, function () {
        return _player.Log.warn(_this4.name, 'init error');
      });
    }
  }, {
    key: "sessionListener",
    value: function sessionListener(session) {
      _player.Log.debug(this.name, 'new session id:' + session.sessionId);
    ...
    value: function sessionUpdateListener() {
      if (this.session) {
        _player.Log.debug(this.name, this.session.status);
    ...
    value: function onSessionTextTracks(tracks) {
      ...
      if (this.textTracks.length > 0) {
        ...
        this.trigger(_player.Events.PLAYBACK_SUBTITLE_AVAILABLE);
    ...
    value: function receiverListener(e) {
      if (e === chrome.cast.ReceiverAvailability.AVAILABLE) {
        _player.Log.debug(this.name, 'receiver found');
        this.show();
      } else {
        _player.Log.debug(this.name, 'receiver list empty');
        this.hide();
      }
    }
  }, {
    ...
    value: function launchSuccess(session) {
      ...
      _player.Log.debug(this.name, 'launch success - session: ' + session.sessionId);
      this.newSession(session);
    }
  }, {
    key: "launchError",
    value: function launchError(e) {
      _player.Log.debug(this.name, 'error on launch', e);
    ...
    value: function loadMediaSuccess(how, mediaSession) {
      var _this5 = this;
      _player.Log.debug(this.name, 'new media session', mediaSession, '(', how, ')');
      this.originalPlayback = this.playback;
      var options = (0, _lodash["default"])({}, this.originalPlayback.options, {
      ...
      this.playbackProxy = new _chromecast_playback["default"](options);
    ...
    value: function loadMediaError(e) {
      _player.Log.warn(this.name, 'media error', e);
    }
  }, {
    key: "newSession",
    value: function newSession(session) {
      ...
      session.addMessageListener("urn:x-cast:".concat(this.messageNamespace, ":text-tracks"), function (_, tracksJSON) {
    ...
    value: function sessionStopped() {
      ...
      if (this.playbackProxy) {
        if (this.playbackProxy.isPlaying() || playerState === 'PAUSED') {
          container.once(_player.Events.CONTAINER_READY, function () {
            container.play();
    ...
    value: function loadMedia() {
      ...
      _player.Log.debug(this.name, 'loading... ' + src);
    ...
    value: function createMediaInfo(src) {
      ...
      mediaInfo.streamType = this.playback.getPlaybackType() === _chromecast_playback["default"].LIVE ? chrome.cast.media.StreamType.LIVE : chrome.cast.media.StreamType.BUFFERED;
    ...
    value: function click() {
      ...
      if (!this.session) {
        var position = 0;
        var connectingIcons = [_ic_cast0_24dp["default"], _ic_cast1_24dp["default"], _ic_cast2_24dp["default"]];
        clearInterval(this.connectAnimInterval);
    ...
    value: function containerPlay() {
      if (this.session && (!this.mediaSession || this.mediaSession.playerState === 'IDLE' || this.mediaSession.playerState === 'PAUSED')) {
        _player.Log.debug(this.name, 'load media');
        this.loadMedia();
      }
    }
  }, {
    ...
    value: function renderConnected() {
      this.$el.html(_ic_cast_connected_24dp["default"]);
    }
  }, {
    key: "renderDisconnected",
    value: function renderDisconnected() {
      this.$el.html(_ic_cast_24dp["default"]);
    }
  }, {
    key: "render",
    value: function render() {
      ...
      this.$style = _player.Styler.getStyleFor(_style["default"], {
